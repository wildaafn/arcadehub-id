...
export function parseFacilitatorReport(rawText) {
  ...
  headers.forEach((h, idx) => {
    ...
    // Status Google Skills URL Profil (abaikan kolom developer)
    if (
      (norm.includes('status') && (norm.includes('googleskills') || norm.includes('skills'))) ||
      (norm.includes('status') && norm.includes('profil') && !norm.includes('developer'))
    ) {
      colIndex.urlStatus = idx
    }
    // URL Profil Google Skills (abaikan kolom developer)
    else if (
      (norm.includes('url') && (norm.includes('googleskills') || norm.includes('skills'))) ||
      (norm.includes('url') && norm.includes('profil') && !norm.includes('developer')) ||
      norm.includes('publicprofileurl')
    ) {
      colIndex.profileUrl = idx
    }
    // URL Profil Google Developer (opsional)
    else if (norm.includes('url') && norm.includes('developer')) {
      colIndex.devProfileUrl = idx
    }
    // Status URL Profil Google Developer (opsional)
    else if (norm.includes('status') && norm.includes('developer')) {
      colIndex.devUrlStatus = idx
    }
    // Nama Peserta
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const devUrl = colIndex.devProfileUrl !== undefined ? row[colIndex.devProfileUrl] || '' : ''
    const devStatus = colIndex.devUrlStatus !== undefined ? row[colIndex.devUrlStatus] || '' : ''
    ...
    // Status All Good dari laporan Google Skills Boost
    const isStatusGood = /all\s*good|valid|ok|benar/i.test(urlStatus) || (!urlStatus && Boolean(canonicalUrl))
    const isValidUrl = Boolean(canonicalUrl) && isStatusGood
    ...
    participants.push({
      ...
      isValidUrl,
      urlStatus: urlStatus || (isValidUrl ? 'All Good' : 'Wrong Google Skills URL'),
      devProfileUrl: devUrl,
      devUrlStatus: devStatus,
      milestone: rawMilestone,
      milestoneKey,
      bonusMilestone: hasBonus,
      skills: rawSkills,
      has5Skills: rawSkills >= 5,
      games: rawGames,
